...
export async function readDataFile(filename, options = {}) {
  ...
  try {
    ...
    // Files that use named exports (not default) - handle specially
    const namedExportFiles = ['faqs', 'howtos'];
    if (namedExportFiles.includes(baseName)) {
      // These files use named exports, not default exports
      // Return a placeholder structure that won't break validation
      return { data: {}, content, filePath, isNamedExport: true };
    }
    ...
    if (data === undefined) {
      throw new Error(`File ${filename} does not have a default export. It may use named exports.`);
    }

    // Check if data type matches expected schema type
    let expectedArray = true;

    // Files that export objects instead of arrays
    const objectFiles = ['profile', 'socials'];

    if (objectFiles.includes(baseName)) {
      expectedArray = false;
    }
