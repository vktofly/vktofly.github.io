...
export async function loadMarkdownFromPath(absoluteOrRelativePath) {
  ...
  // Note: Turbopack warning TP1004 about dynamic paths is expected here.
  // This function intentionally handles dynamic paths for server-side markdown loading.
  // Paths are resolved at build time during static generation, making this safe.
  let resolvedPath;
  if (path.isAbsolute(absoluteOrRelativePath)) {
    resolvedPath = absoluteOrRelativePath;
  } else {
    const cwd = process.cwd();
    resolvedPath = path.resolve(cwd, absoluteOrRelativePath);
  }

  // Read and process markdown file
  // The path resolution happens at build time for static generation
