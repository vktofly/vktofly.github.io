...
export default function PersonImageUploader({ people: peopleProp }) {
  const [people] = useState(peopleProp || []);
  ...
  const handleFile = async (file) => {
    ...
    reader.onload = (e) => {
      ...
      setUploadedImages({
        ...uploadedImages,
        [selectedPerson.slug]: base64Image,
      });
    };
  ...
  const generateUpdatedFile = () => {
    const updatedPeople = people.map((person) => {
      if (uploadedImages[person.slug]) {
        return {
          ...person,
          image: uploadedImages[person.slug],
        };
      }
      return person;
    });
...
`;
    ...
    const fileContent = `${header}const influentialPeople = ${JSON.stringify(updatedPeople, null, 2)};
...
`;
  ...
  const copyToClipboard = () => {
    if (generatedFile) {
      ...
      alert("File content copied to clipboard! Paste it into data/influentialPeople.js");
    }
  };
  ...
  const downloadFile = () => {
    if (generatedFile) {
      ...
      a.href = url;
      a.download = "influentialPeople.js";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 border border-zinc-200 dark:border-zinc-800 rounded-lg bg-white dark:bg-zinc-900">
        ...
        <select
          value={selectedPerson?.slug || ""}
          onChange={(e) => {
            const person = people.find((p) => p.slug === e.target.value);
            setSelectedPerson(person);
            setImageUrl(person?.image || uploadedImages[person?.slug] || "");
          }}
        ...
        <>
          ...
          {selectedPerson.image && (
            <div className="p-6 border border-zinc-200 dark:border-zinc-800 rounded-lg bg-white dark:bg-zinc-900">
              ...
              <div className="relative w-32 aspect-square rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-800">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-medium text-palette-primary dark:text-zinc-200">
                Updated {Object.keys(uploadedImages).length} person
      ...
      {generatedFile && (
        <div className="p-6 border border-zinc-200 dark:border-zinc-800 rounded-lg bg-white dark:bg-zinc-900">
          ...
          <p className="text-xs text-palette-secondary dark:text-zinc-400 mt-2">
            Replace the content of <code className="bg-zinc-100 dark:bg-zinc-800 px-1 py-0.5 rounded">data/influentialPeople.js</code> with
            this content
          </p>
        </div>
      )}
